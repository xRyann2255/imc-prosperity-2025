...
class Trader:
    ...
    def squid_ink_mean_reversion(
            ...
            if cooldown_elapsed:
                ...
                if z_score > self.squid_z_threshold and order_book_imbalance < -0.1 and position > -position_limit * 0.8:
                    ...
                    if sell_quantity > 0:
                        ...
                        logger.print(f"SQUID_INK: SELLING {sell_quantity} at {best_bid} (z-score: {z_score:.2f}, imbalance: {order_book_imbalance:.2f}, mean reversion)")
                        ...
                        trade_state["positions"].append({
                            ...
                            "stop_loss_price": best_bid * 1.04  # 4% stop loss for shorts
                        })
                        trade_state["last_trade_time"] = timestamp
                
                # Strong mean reversion signal - price significantly below long-term mean
                elif z_score < -self.squid_z_threshold and order_book_imbalance > 0.1 and position < position_limit * 0.8:
                    ...
                    if buy_quantity > 0:
                        ...
                        logger.print(f"SQUID_INK: BUYING {buy_quantity} at {best_ask} (z-score: {z_score:.2f}, imbalance: {order_book_imbalance:.2f}, mean reversion)")
                        ...
                        trade_state["positions"].append({
                            ...
                            "z_score": z_score,
                            "target_price": ema,  # Target the long-term mean
                            "stop_loss_price": best_ask * 0.96  # 4% stop loss for longs
                        })
                        trade_state["last_trade_time"] = timestamp
            ...
            if trade_state["positions"]:
                ...
                for pos in trade_state["positions"]:
                    if pos["type"] == "LONG":
                        # Check stop loss - exit if price falls below stop loss
                        if mid_price <= pos["stop_loss_price"]:
                            # Sell to close the long position at a loss
                            sell_quantity = pos["quantity"]
                            orders.append(Order(symbol, best_bid, -sell_quantity))
                            logger.print(f"SQUID_INK: STOP LOSS - Selling {sell_quantity} at {best_bid} (stop price: {pos['stop_loss_price']})")
                        # Price has reverted toward mean - take profit
                        elif mid_price >= pos["target_price"] or z_score >= -0.5:
                        ...
                        # Check stop loss - exit if price rises above stop loss
                        if mid_price >= pos["stop_loss_price"]:
                            # Buy to close the short position at a loss
                            buy_quantity = pos["quantity"]
                            orders.append(Order(symbol, best_ask, buy_quantity))
                            logger.print(f"SQUID_INK: STOP LOSS - Buying {buy_quantity} at {best_ask} (stop price: {pos['stop_loss_price']})")
                        # Price has reverted toward mean - take profit
                        elif mid_price <= pos["target_price"] or z_score <= 0.5:
            ...
            if position_pct > 0.8:
                if position > 0:  # We have a large long position
                    ...
                    if reduce_quantity > 0:
                        ...
                        # Update positions list by reducing quantities proportionally
                        total_pos_quantity = sum(p["quantity"] for p in trade_state["positions"] if p["type"] == "LONG")
                        if total_pos_quantity > 0:
                            reduction_factor = (total_pos_quantity - reduce_quantity) / total_pos_quantity
                            for pos in trade_state["positions"]:
                                if pos["type"] == "LONG":
                                    pos["quantity"] = max(1, int(pos["quantity"] * reduction_factor))
                    ...
                    if reduce_quantity > 0:
                        ...
                        # Update positions list by reducing quantities proportionally
                        total_pos_quantity = sum(p["quantity"] for p in trade_state["positions"] if p["type"] == "SHORT")
                        if total_pos_quantity > 0:
                            reduction_factor = (total_pos_quantity - reduce_quantity) / total_pos_quantity
                            for pos in trade_state["positions"]:
                                if pos["type"] == "SHORT":
