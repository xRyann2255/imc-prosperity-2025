...
class Trader:
    def __init__(self):
        ...
        # Constants based on data analysis
        self.SQUID_INK_MEAN = 1907.84
    ...
    def calculate_z_score(self, current_price, prices, ma_period):
        """Calculate z-score (deviation from mean in standard deviations)"""
        if len(prices) < ma_period:
            return 0
            
        # Calculate moving average
        ma = self.calculate_moving_average(prices, ma_period)
        
        # Calculate standard deviation
        variance = sum((p - ma) ** 2 for p in prices[-ma_period:]) / ma_period
    ...
    def squid_ink_mean_reversion(
        ...
        ma_short = self.calculate_moving_average(prices, self.squid_ma_short)
        ma_long = self.calculate_moving_average(prices, self.squid_ma_long)
