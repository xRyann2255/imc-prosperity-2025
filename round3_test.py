...
class BlackScholes:
    def black_scholes_call(spot, strike, time_to_expiry, volatility):
        d1 = (
            math.log(spot / strike) + (0.5 * volatility ** 2) * time_to_expiry
    ...
    def implied_volatility(call_price, spot, strike, time_to_expiry, max_iterations=200, tolerance=1e-10):
        ...
        for _ in range(max_iterations):
            ...
            estimated_price = BlackScholes.black_scholes_call(spot, strike, time_to_expiry, mid_vol)
...
class VolcanoSpread(MarketMakingStrategy):
    def __init__(self):
    ...
    def get_vol_spread(self, state: TradingState):
        try:
            p250 = self.mid_price(state.order_depths["VOLCANIC_ROCK_VOUCHER_10250"])
            p500 = self.mid_price(state.order_depths["VOLCANIC_ROCK_VOUCHER_10500"])
            spot = state.observations.conversionObservations["VOLCANIC_ROCK"].midPrice
            if not (p250 and p500 and spot):
                return None, None, None

            iv250 = BlackScholes.implied_volatility(p250, spot, self.s250, self.expiration)
            iv500 = BlackScholes.implied_volatility(p500, spot, self.s500, self.expiration)
            return iv500 - iv250, iv250, iv500
        except:
            return None, None, None

    def act(self, state: TradingState):
        orders = []
        spread, iv250, iv500 = self.get_vol_spread(state)
        if spread is None:
            return []
        ...
        m = mean(self.window)
        s = stdev(self.window)
        z = (spread - m) / s if s != 0 else 0

        # Position limits
        pos_250 = state.position.get("VOLCANIC_ROCK_VOUCHER_10250", 0)
        pos_500 = state.position.get("VOLCANIC_ROCK_VOUCHER_10500", 0)
        pos_underlying = state.position.get("VOLCANIC_ROCK", 0)

        # Trade size
        size = 160

        if z > self.z_high:
            orders.append(Order("VOLCANIC_ROCK_VOUCHER_10500", min(state.order_depths["VOLCANIC_ROCK_VOUCHER_10500"].buy_orders), -size))
            orders.append(Order("VOLCANIC_ROCK_VOUCHER_10250", max(state.order_depths["VOLCANIC_ROCK_VOUCHER_10250"].sell_orders), size))
        elif z < self.z_low:
            orders.append(Order("VOLCANIC_ROCK_VOUCHER_10500", max(state.order_depths["VOLCANIC_ROCK_VOUCHER_10500"].sell_orders), size))
            orders.append(Order("VOLCANIC_ROCK_VOUCHER_10250", min(state.order_depths["VOLCANIC_ROCK_VOUCHER_10250"].buy_orders), -size))

        # Delta hedge
        net_delta = (
            pos_250 * BlackScholes.delta(spot, self.s250, self.expiration, iv250)
            + pos_500 * BlackScholes.delta(spot, self.s500, self.expiration, iv500)
        )
        hedge_needed = -round(net_delta - self.last_hedged_delta)
        self.last_hedged_delta = net_delta

        underlying_depth = state.order_depths["VOLCANIC_ROCK"]
        if hedge_needed > 0:
            orders.append(Order("VOLCANIC_ROCK", min(underlying_depth.sell_orders), hedge_needed))
        elif hedge_needed < 0:
            orders.append(Order("VOLCANIC_ROCK", max(underlying_depth.buy_orders), hedge_needed))
...
class BasketHedgeStrategy(Strategy):
    ...
    def act(self, state: TradingState) -> None:
        ...
        # Instead of sweeping the order book, we only trade at the best available price level.
        if delta > 0:
            # Need to buy 'delta' units using only the best ask.
            if not od.sell_orders:
                return
            best_ask = min(od.sell_orders.keys())
            available = -od.sell_orders[best_ask]  # volume is stored as negative for sell orders
            trade_qty = min(delta, available)
            if trade_qty > 0:
                self.buy(best_ask, trade_qty)
                logger.print(f"Buying {trade_qty} at best ask {best_ask}")
        else:
            # delta < 0: Need to sell abs(delta) units using only the best bid.
            if not od.buy_orders:
                return
            best_bid = max(od.buy_orders.keys())
            available = od.buy_orders[best_bid]
            trade_qty = min(abs(delta), available)
            if trade_qty > 0:
                self.sell(best_bid, trade_qty)
                logger.print(f"Selling {trade_qty} at best bid {best_bid}")
    ...
    def __init__(self) -> None:
        ...
        self.strategies = {
            ...
            "SQUID_INK": SquidInkDeviationStrategy("SQUID_INK",
                                                   ...
                                                   low_trade_ratio=0.5),
            "VOLCANIC_ROCK_VOUCHER_10250": VolcanoSpread(),
