...
class VolcanicOptionVolatilityStrategy(Strategy):
    ...
    4. Trades when the price difference exceeds a threshold
    """
    ...
    def __init__(self, 
                 ...
                 entry_threshold: float = 5.0,  # Price difference threshold for entry
                 exit_threshold: float = 2.0,  # Price difference threshold for exit
                 ...
                 smile_window_size: int = 100,  # Number of timestamps to keep in volatility smile history
                 vol_history_size: int = 50,  # Number of volatility observations to keep for each strike
    ...
    def interpolate_volatility(self, moneyness: float) -> Optional[float]:
        """
        Interpolate implied volatility from the volatility smile at a given moneyness.
        Uses linear interpolation between points.
        """
        smile = self.get_current_smile()
        if not smile:
        ...
        # Check if moneyness is outside the range of the smile
        if moneyness < smile[0][0]:
            return smile[0][1]  # Return the leftmost volatility
        elif moneyness > smile[-1][0]:
            return smile[-1][1]  # Return the rightmost volatility
        
        # Find the two points to interpolate between
        for i in range(len(smile) - 1):
            if smile[i][0] <= moneyness <= smile[i+1][0]:
                m1, v1 = smile[i]
                m2, v2 = smile[i+1]
                ...
                if m2 == m1:  # Avoid division by zero
                    return v1
                return v1 + (v2 - v1) * (moneyness - m1) / (m2 - m1)
        
        return self.base_volatility  # Fallback
    ...
    def act(self, state: TradingState) -> None:
        ...
        # Determine trading action based on price difference
        if abs(price_diff) > self.entry_threshold:
            if price_diff > 0:  # Option is overpriced - sell it
                # Don't exceed short position limit
                qty_to_sell = min(
                ...
                # Don't exceed long position limit
                qty_to_buy = min(
        ...
        if self.debug and self.orders:
            ...
            logger.print(f"  Market price: {option_price}, Theoretical: {theoretical_price:.2f}, Diff: {price_diff:.2f}")
...
class VolatilityHedger:
    ...
    def run(self, state: TradingState) -> List[Order]:
        ...
        for symbol in self.option_symbols:
            ...
            if self.debug:
                logger.print(f"[{symbol}] Position: {position}, Strike: {strike}, Delta: {delta:.4f}")
    ...
    def __init__(self) -> None:
        ...
        self.strategies = {
            ...
            "VOLCANIC_ROCK_VOUCHER_9500_VOL": VolcanicOptionVolatilityStrategy(
                ...
                entry_threshold=10.0,  # Price difference threshold for entry
                exit_threshold=5.0,    # Price difference threshold for exit
