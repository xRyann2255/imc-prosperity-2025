...
class SquidInkPredictor:
    ...
    def __init__(self):
        # Trained AR(2) model coefficients from our analysis
        self.coefficients = [0.79662288, 0.20251730]  # Trained lag 1 and lag 2 coefficients
        self.intercept = 1.70983117  # Trained intercept
        
    def predict_next_price(self, prices):
        """
        Predict the next price using the AR(2) model
        
        Args:
            prices: List of recent prices, with most recent last
                   Need at least 2 prices for our AR(2) model
                   
        Returns:
            Predicted next price (as integer)
        """
        if len(prices) < 2:  # Need at least 2 prices for AR(2)
            return int(prices[-1])  # Return last known price if not enough data
            
        # Get the last 2 prices (most recent first)
        recent_prices = prices[-2:]
        
        # Apply AR formula: intercept + coef1*price[t-1] + coef2*price[t-2]
    ...
    def predict_direction(self, prices):
        """
        ...
        """
        if len(prices) < 3:  # Need 2 for prediction + 1 for current
            return 0
            
        current_price = prices[-1]
        # Get the most recent prices for the model
        recent_prices = prices[-3:-1]  # Get the 2 prices before the current one
        predicted_price = self.predict_next_price(recent_prices)
    ...
    def get_confidence(self, prices):
        ...
        For the AR(2) model, use the directional accuracy from training (0.6365)
        
        Returns:
            Confidence score between 0 and 1
        """
        # Fixed confidence based on our model's directional accuracy
        return 0.6365
...
class Trader:
    ...
    def squid_ink_orders(self, order_depth: OrderDepth, position: int, position_limit: int) -> List[Order]:
        ...
        if len(self.squid_prices) < 2:  # Need at least 2 prices for AR(2) model
            return []  # Not enough data yet
        
        # Get price prediction from trained model
        recent_prices = list(self.squid_prices)[-2:]  # Get last 2 prices for AR(2) model
        ...
        # Make sure price levels are integers
        baaf = int(baaf)
        bbbf = int(bbbf)
        ...
        # Place resting orders - size based on confidence and direction
        buy_quantity = position_limit - (position + buy_order_volume)
        if buy_quantity > 0 and (price_direction >= 0 or position < 0):  # Buy if direction is up or we need to cover short
            ...
            if order_size > 0:
                ...
                orders.append(Order("SQUID_INK", bid_price, order_size))
        
        sell_quantity = position_limit + (position - sell_order_volume)
        if sell_quantity > 0 and (price_direction <= 0 or position > 0):  # Sell if direction is down or we need to reduce long
            # Size order based on confidence
            order_size = int(sell_quantity * (0.5 + 0.5 * confidence))
            if order_size > 0:
                # Make sure price is an integer
                ask_price = int(baaf - 1)
                orders.append(Order("SQUID_INK", ask_price, -order_size))
